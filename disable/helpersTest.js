...
describe('Helpers', function() {
    it('should return json data', function(done) {
        helpers.getJSON("https://419webdev.site/titles").then(function(data){
            expect(data).to.deep.equal('[{"zFile":"zork.z5"},{"zFile":"zork2.z5"},{"zFile":"zork3.z5"},{"zFile":"ztuu.z5"}]');
            done();
        }).catch();
    });
    it('should return valid xml data', function(done) {
        helpers.getXML("https://www.w3schools.com/xml/note.xml").then(function(data){
    ...
    it('should throw error if iso date is not valid', function() {
        assert.throws(function () { helpers.validate_iso8601("2014-01-01T23:59") }, "Incorrect date format, should be YYYY-MM-DDTHH:mm:ss");
